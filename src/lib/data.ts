...
export const INITIAL_PROMPTS: PromptAction[] = [
  ...
  // --- Workflow A, Step 1: Section C — Other Modeling Key Inputs -------------
  {
    id: "A-003",
    title: "Projection Horizon",
    category: "Other Modeling Key Inputs",
    step: "Workflow A – Step 1: Template Selection & Key Inputs",
    status: "Active",
    lastUpdated: "2026-09-15",
    description:
      "Section C.1 — sets the number of projection years, per sub-sector when Sum-of-the-Parts is selected.",
    variables: ["{{projection_years}}", "{{projection_years_by_line}}"],
    promptText:
      "Set the projection horizon of the standardized model to {{projection_years}} years. Extend or shorten the projection columns of every statement, schedule and valuation tab so they all cover exactly this horizon, keeping formulas and the historical/projection boundary intact. Under Sum-of-the-Parts, apply {{projection_years_by_line}} to each sub-sector's Master Model and Valuation tab and align the consolidated rollup to the longest horizon." +
      NO_GUESSING,
    requiredResources: [],
  },
  {
    id: "A-004",
    title: "Working Capital Days Basis",
    category: "Other Modeling Key Inputs",
    step: "Workflow A – Step 1: Template Selection & Key Inputs",
    status: "Active",
    lastUpdated: "2026-09-15",
    description:
      "Section C.2 — applies the selected historical basis (or manual input) for each working capital asset and liability.",
    variables: ["{{working_capital_basis}}", "{{working_capital_basis_by_line}}"],
    promptText:
      "Configure the working capital schedule using {{working_capital_basis}}: for each asset and liability line item, derive the days assumption from the selected historical basis (for example last year, or an average of a number of historical years) or use the number of days entered manually. Keep each line item's existing driver linkage (revenue, COGS or the template's default) unchanged. Under Sum-of-the-Parts, apply {{working_capital_basis_by_line}} per sub-sector." +
      NO_GUESSING,
    requiredResources: [],
  },
  {
    ...
    lastUpdated: "2026-09-15",
    description:
      "Section C.3 — sizes the comps set used for beta, multiples and the relative valuation tab.",
    variables: ["{{comps_count}}", "{{comps_count_by_line}}"],
    promptText:
      "Prepare the comparable companies block for {{comps_count}} comps: add or remove comp rows in the comps and beta schedules so the count matches, and keep the multiple, beta and weighted-average formulas referencing the full range. Under Sum-of-the-Parts, build a separate comps block per sub-sector using {{comps_count_by_line}}. Leave the comp names and market data blank unless supplied in the uploaded documents or the market data resources." +
      NO_GUESSING,
    requiredResources: [],
  },
  {
    id: "A-006",
    title: "Share Classes and Preferred Share Rights",
    category: "Other Modeling Key Inputs",
    step: "Workflow A – Step 1: Template Selection & Key Inputs",
    status: "Active",
    lastUpdated: "2026-09-15",
    description:
      "Section C.4 — configures IFC's common and/or preferred shares and reflects the described preferred rights in the equity waterfall.",
    variables: ["{{share_classes}}", "{{preferred_share_rights}}"],
    promptText:
      "Configure IFC's holding as {{share_classes}}. For common shares only, value the stake pro rata to equity value. When preferred shares are present, add the preferred layer to the equity waterfall and reflect the rights described in {{preferred_share_rights}} (for example liquidation preference, dividend accrual and conversion terms), then value IFC's stake as the higher of the preferred entitlement and the converted common value where the terms allow conversion. If the rights described are not sufficient to model the waterfall, flag the missing terms instead of assuming market-standard ones." +
      NO_GUESSING,
    requiredResources: [],
  },
  {
    ...
    lastUpdated: "2026-09-15",
    description:
      "Section C.5 — adds the liquidity put tab and the selected put-price mechanisms.",
    variables: ["{{liquidity_put}}", "{{put_price_mechanisms}}"],
    promptText:
      "If {{liquidity_put}} is No, leave the put schedule out of the model. If Yes, add the liquidity put schedule and compute the put price using each mechanism listed in {{put_price_mechanisms}}, calculating them side by side and taking the maximum (or the combination the agreement specifies) as the put value. Compare the put value with the DCF equity value for IFC's stake and carry the higher of the two into the valuation summary. Source the put terms — strike basis, exercise window, floors and caps — from the supplied agreements only." +
      NO_GUESSING,
    requiredResources: [],
  },
  // --- Workflow A, Step 1: Section D — FX Key Inputs -------------------------
  {
    id: "A-008",
    ...
    lastUpdated: "2026-09-15",
    description:
      "Section D.1 — sets the countries (per sub-sector under SOTP) and their local currencies, grouping the tail when there are more than three.",
    variables: ["{{country_count}}", "{{countries}}", "{{countries_by_line}}"],
    promptText:
      "Set up the geographic dimension of the model for {{country_count}} country(ies) using {{countries}} and their local currencies. Create one local-currency input column per country in the revenue, cost and FX schedules. When more than three countries are indicated, model the top three individually and group the remainder into a single 'Other countries' category that uses the FX rate of the main country of operations. Under Sum-of-the-Parts, apply {{countries_by_line}} so each sub-sector carries its own country set and currencies." +
      NO_GUESSING,
    requiredResources: [],
  },
  {
    id: "A-009",
    title: "Foreign Currency Exposure (Single-Country Companies)",
    category: "Other Modeling Key Inputs",
    step: "Workflow A – Step 1: Template Selection & Key Inputs",
    status: "Active",
    lastUpdated: "2026-09-15",
    description:
      "Section D.1 follow-up — adds import/export FX exposure for companies operating in a single country.",
    variables: ["{{has_foreign_currency}}"],
    promptText:
      "This instruction applies only when the company operates in a single country. If {{has_foreign_currency}} is Yes, add a USD exposure block that splits revenues, costs and capital expenditure between local currency and USD, and drive the USD portion off the FX schedule so devaluation flows through margins and cash flow. If No, keep the model entirely in local currency and leave the FX exposure block out. Take the local/USD split from the supplied documents; do not assume a split." +
      NO_GUESSING,
    requiredResources: [],
  },
  {
    id: "A-010",
    title: "Reporting Currency and FX Conversion",
    category: "Other Modeling Key Inputs",
    step: "Workflow A – Step 1: Template Selection & Key Inputs",
    status: "Active",
    lastUpdated: "2026-09-15",
    description:
      "Section D.2/D.3 — sets the reporting currency and the translation of local-currency flows into it.",
    variables: ["{{reporting_currency}}", "{{countries}}"],
    promptText:
      "Set the model's reporting currency to {{reporting_currency}}. Label every statement, schedule and valuation tab in this currency and translate each local-currency flow from {{countries}} into it through the FX schedule: average rates for income statement and cash flow items, closing rates for balance sheet items. Keep the discount rate, comps multiples and terminal value consistent with the reporting currency, and note where inflation differentials require a real-versus-nominal adjustment. Use FX rates from the market data resources supplied to the model." +
      NO_GUESSING,
    requiredResources: [],
  },
  {
